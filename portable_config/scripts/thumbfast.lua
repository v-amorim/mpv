...
-- Local patch: upstream passes mpv_path straight to subprocess, so ~~/ never expands.
if options.mpv_path:find("~") then
	options.mpv_path = mp.command_native({ "expand-path", options.mpv_path })
end
...
local function spawn(time)
	...
	if subprocess_timeout then
		subprocess_timeout:kill()
	end

	subprocess_timeout = mp.add_timeout(10, function()
		mp.msg.error("thumbfast: subprocess timeout triggered - killing hanging subprocess")
		if spawn_waiting then
			spawned = false
			spawn_waiting = false
		end
	end)
